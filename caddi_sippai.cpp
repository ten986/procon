...
int ycomp(sphere& a, sphere& b) {
	return (a.point==b.point)?(a.radius>b.radius):(a.point > b.point);
}
...
int main()
{
  ...
  REP(i,N){
    spheres[i].bonus_dict = vector<int>(1000, 0);
    REP(j,spheres[i].points.size()){
      spheres[i].bonus_dict[spheres[i].points[j].target] += spheres[i].points[j].point;
    }
  }
  ...
  REP(i,spheres.size()){
    ...
    if(flag){continue;}
    if(spheres[i].radius<=size_limit4){
      sphere newsphere(spheres[i].number,spheres[i].radius,spheres[i].point);
      REP(j,spheres[i].points.size()){
        bonus targ = spheres[i].points[j];
        newsphere.points.push_back(targ);
  ...
  REP(i,spheres.size()){
    ...
    limit_spheres6.push_back(newsphere);
  }

  YSORT(limit_spheres6);

  int radius_limit=999999;

  REP(i,limit_spheres6.size()){
    sphere s=limit_spheres6[i];
    ...
    REP(times,100000){
      ...
      if(canput){
        stage.push_back(xyzrn(x1.x,x1.y,x1.z,s.radius,s.number));
        flag=true;
        break;
      }
    }
    if(!flag){
      ...
      radius_limit=max(radius_limit,60);
    }
  }
